module ProjectsHelper
  def project_menu(project)
    "".html_safe.tap do |output|
      output << link_to('<i class=" icon-download-alt"></i> Download'.html_safe, export_project_path(project), :class => 'btn', :title => 'Download Offline Copy', :id => 'project_export_button') + ' ' unless offline_mode? || !current_user.can_download
